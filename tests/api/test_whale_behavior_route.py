...
@pytest.mark.asyncio
async def test_token_whale_route_collapses_duplicate_shared_annotations(monkeypatch):
    whale._whale_cache.clear()
    whale._whale_cache_locks.clear()
    ...
    shared_flags = [{"code": "liquidity_drain", "severity": "high"}]
    shared_heuristics = [{"code": "deployer_retained_supply", "severity": "medium", "confidence": 0.8}]
            ...
            return [
                {
                    ...
                    "timestamp": "2026-03-18T00:00:00+00:00",
                    "dex_name": "Raydium",
                    "anomaly_flags": shared_flags,
                    "entity_heuristics": shared_heuristics,
                },
                {
                    "signature": "sig-2",
    ...
    request = SimpleNamespace(match_info={"address": "token-1"}, query={"force_refresh": "1"})

    response = await whale.get_whale_activity_for_token(request)
    payload = json.loads(response.text)

    assert len(payload["behavior"]["anomaly_flags"]) == 1
    assert len(payload["behavior"]["entity_heuristics"]) == 1
