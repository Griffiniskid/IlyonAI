...
export interface PoolLinkPayload {
  ...
  notice?: string;
  pool_kind?: "v2" | "v3" | "stable" | "vault" | string;
}
...
const KIND_BADGES: Record<string, { label: string; tone: string }> = {
  v3:     { label: "Concentrated", tone: "border-violet-500/30 bg-violet-500/10 text-violet-300" },
  v2:     { label: "Full-range AMM", tone: "border-cyan-500/30 bg-cyan-500/10 text-cyan-300" },
  stable: { label: "Stable",       tone: "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" },
  vault:  { label: "Auto-vault",   tone: "border-amber-500/30 bg-amber-500/10 text-amber-300" },
};
  ...
  const sentinelScore =
    payload.sentinel && typeof payload.sentinel === "object"
      ? (payload.sentinel as Record<string, number>).opportunity_score ??
        ...
        null
      : null;
  const kind = payload.pool_kind ? KIND_BADGES[payload.pool_kind] : null;
  ...
  return (
    <div className="rounded-xl border border-amber-500/30 bg-gradient-to-br from-slate-900 via-slate-900/95 to-amber-950/40 p-4 shadow-lg">
      ...
      <div className="mb-3 flex items-start gap-2 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
        ...
        <span>
          {payload.notice ||
            "Direct pool execution requires extra steps. Open this pool on the protocol app to deposit."}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {kind && (
              <span className={`rounded-md border px-1.5 py-0.5 text-[10px] uppercase tracking-wide ${kind.tone}`}>
                {kind.label}
              </span>
            )}
          </div>
      ...
      <div className="mt-3 grid grid-cols-3 gap-2">
        <div className="rounded-lg border border-slate-800 bg-slate-900/60 px-3 py-2">
          <div className="text-[10px] uppercase tracking-wide text-slate-500">APY</div>
        ...
        <div className="rounded-lg border border-slate-800 bg-slate-900/60 px-3 py-2">
          ...
          <div className="mt-0.5 text-sm font-semibold text-slate-200">{fmtTvl(payload.tvl_usd ?? null)}</div>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-900/60 px-3 py-2">
          ...
          <div className="mt-0.5 truncate text-sm font-semibold text-slate-200">
            {payload.amount && payload.asset_in ? `${payload.amount} ${payload.asset_in}` : "—"}
          </div>
        </div>
      </div>
      ...
      >
        <div>
          <div className="text-sm font-semibold text-emerald-300">Open on {proto}</div>
          <div className="text-[11px] text-emerald-400/70">
            {shortHost(payload.url)} — finalize deposit on the protocol app
          </div>
        </div>
