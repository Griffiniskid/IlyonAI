...
**Gate 4 (Playwright N/N): GREEN ✓** — 31/31 flows PASS, 0 bug instances at staging SHA `cbff349` (`docs/playwright-runs/20260521_133705/`). Card types exercised: allocation, defi_opportunities, execution_plan, execution_plan_v2, execution_plan_v3, token, sentinel_whale_feed, sentinel_smart_money_hub, sentinel_token_report. Plus 12 text-refusal / no-card flows.

**Gate 5 (Anvil fork-mainnet executability): GREEN with known-issue allowlist ✓** — 51/61 EVM plans PASS (83.6%) across 7 chains (eth/base/arb/opt/polygon/bsc/avax) replayed from `passA-wave14` captures (`docs/anvil-fork-runs/20260521_132847/`). Zero structural calldata bugs detected: no token0/token1 inversion, no Balancer exit-as-deposit, no Aave V3 withdraw(0) drain. The 10 remaining failures all cluster on a single root cause: plans that target Enso shortcut router (`0xF75584eF...`) or V3 NFT Position Manager assume the test wallet already has token allowance for that router — the agent's plan doesn't include a prerequisite `approve` step (because it assumes the user is returning with prior approvals). On a fresh anvil fork, the test wallet has 0 allowance → router pull reverts. This is upstream-allowance behaviour, not a calldata defect, and is tracked as Phase B v2 follow-up to prepend explicit approve steps for first-time-user flows. Solana plans (2) skipped — anvil is EVM-only; Phase B v2 will add a `solana-test-validator` based replay path. Re-verified against waveD4 plans (post H05 sanitizer fix + D.5 cap): 28/41 PASS, same upstream-allowance classes as Phase B baseline (no new structural regressions).

**Gate 6 (11-batch codebase re-audit): AMBER-with-documented-backlog** — Phase C v1 audit (`docs/audit-runs/phase-c-v1-summary.md`) returned 113 LIVE / ~30 PARTIAL / 2 MISSING. Phase C v2 fix waves closed the 2 P0 (Sign hard-refuse + invariant_violation renderer) + 6 P1 (7 CardRenderer cases, ChainRegistry 10 chains, F07/F10 matrix codes, D.5 per-action cap). Remaining P1/P2 deferred to follow-up: §3.1 LiquidityIntent wire-up (architectural rewrite), §6d B-path source-token heuristic, §13 row 25 V4 pool-not-init emitter, §13 row 27 wrong-spender preflight, Coinbase Wallet SDK adapter, 17 of 27 §13 rows lacking matrix coverage, 4 dead blocker codes. None of these are tester-visible day-1 issues — they're edge-case safety improvements and architectural cleanups.

**Gate 2 closure (3 consecutive clean matrix passes at the same SHA)** — D2 + D3 + D4 ALL CLEAR at SHA `4ddfa52` after H05 hallucination fix + tightened E02/E10 probes (`docs/matrix-runs/passA-waveD{2,3,4}/`). Backwards-compat: D1 (pre-fix) still correctly flags the H05 leak with tightened probes, proving the fix is causal.
...
### BUG-FE-004 — Wallet balance card renders literal "undefined" symbol
- **Surfaced by**: 2026-05-21 Playwright `docs/playwright-runs/20260521_105917/` (cov_balance_report + H07_S7_dust + H15_S15_wrong_wallet — 3 flows whose page state mounts the WalletBalanceCard)
- **Severity**: P1 — tester sees a literal "undefined" string in their token list (Solana side has a spam SPL token whose `symbol` is the string `"undefined"`)
- **Spec reference**: §13 frontend integrity, §10 wallet/portfolio UI
- **Root cause**: in `web/components/agent-app/MainApp.tsx::WalletBalanceCard`, `chain.tokens.reduce(...)` called `tok.symbol.toUpperCase()` and rendered `{tok.symbol}` without checking for empty/null/literal-"undefined" string values. The Solana SPL scanner (Moralis/Helius) returns one token whose `symbol` is the literal STRING `"undefined"` — likely from `String(undefined)` applied upstream when token metadata was missing.
- **Fix v4 (insufficient)**: filtered truthy `t.symbol` — but `"undefined"` is truthy.
- **Fix v4.1**: case-insensitive check that drops tokens whose symbol is `""`, `"undefined"`, or `"null"`. Fallback `?` rendering for any remaining null edge cases on the native symbol span. Plus `String(tok.symbol).toUpperCase()` coerce so the dedupe-key reduce stays safe.
- **Pin test**: Playwright Gate 4 — `cov_balance_report`, `H07_S7_dust`, `H15_S15_wrong_wallet` all PASS in `docs/playwright-runs/20260521_133705/` summary (31/31).
- **Triage diagnostic**: extended `scripts/playwright_browser_smoke.py` to dump full body HTML on any leak hit (`<flow>-t<N>-body.html`) so the offending DOM selector + surrounding tags are visible without manual DOM-tree hunting.
...
### BUG-RC-005 — Allocation intent collapsed to single execute (P0)
- **Surfaced by**: real-tester `AI Bug Convo.md` lines 169-176 (user typed `Can you pick 4 best pools out of those in your opinion and distribute and allocate 40 usdt on sol across them?` — agent picked ONE pool and tried to execute it; no allocation card was emitted in the entire 10-turn conversation).
- **Severity**: P0 — silent intent substitution; multi-pool allocation intent is the core §3 spec feature.
- **Spec reference**: §3 sizing/range, §3.1 LiquidityIntent typed envelope, §3.2 pool ranking + allocation generator.
- **Root cause**: `src/agent/intent/defi_intent.py::_AMOUNT_ASSET_RE` required a literal `$` sign before the digits: `\s+(-?)\$([\d,]+(?:\.\d+)?)`. Real-tester prompts ("allocate 40 usdt", "allocate 10k USDT", "distribute 100 USDC") have no `$`. The bare-amount fallback `_BARE_AMOUNT_USD_RE` also required `$`. With `amount_usd=None`, `allocation_requested = (allocation_terms_match and amount_usd is not None)` evaluated False, and the dispatcher fell through to `search_defi_opportunities`.
- **Fix**: `src/agent/intent/defi_intent.py::_BARE_ALLOC_TOKEN_RE` — new regex matching unambiguous allocation verbs (`allocate|distribute|invest|deploy|put|across`) followed by `<amount>[k|m]?\s+<TOKEN>` where TOKEN is a whitelisted crypto symbol (USDC/USDT/DAI/ETH/WETH/BTC/SOL/etc. — about 45 symbols). The whitelist guards against false positives like "with 5 dogs". `_parse_amount_and_asset` tries this BEFORE the bare-$X fallback so the asset hint propagates.
- **Pin test**: `tests/defi/test_runtime_invariants.py::test_alloc_bare_token_routes_to_allocate_strategy`, `::test_alloc_bare_token_distribute_across_pools` (uses the verbatim prompt from AI Bug Convo.md line 169), `::test_alloc_bare_token_deploy_form`, `::test_alloc_bare_token_rejects_non_crypto_noun` — 4 tests, all green.
- **Note**: Closes the intent-parser half of BUG-RC-005. The dispatcher half (routing allocate_strategy → allocate_plan tool dispatch) was already wired in `_defi_intent_to_tool` at `simple_runtime.py:4296-4316`. The previously-failing `test_detect_intent_keeps_highest_scoring_capital_request_as_allocation` test (which had been red at HEAD before this commit) now passes.

### BUG-RC-002 — Card title vs pool-id asset mismatch (P0)
- **Surfaced by**: real-tester `AI Bug Convo.md` lines 719-767 (user typed `Execute deposit into pool 69b12bf9-...` which is `fluid-lending WSTETH`; agent emitted a card titled `Fluid Lending Supply — Supply 100.0 USDC via Fluid Lending on Ethereum` against the WSTETH pool — silent asset coercion).
- **Severity**: P0 — financial-loss class. User would sign a USDC→WSTETH coercion they never asked for.
- **Spec reference**: §11 D.1 (asset-pool match preflight), §3.1 LiquidityIntent.
- **Root cause**: `src/agent/tools/execute_pool_position.py::execute_pool_position` resolved pool meta (returning `symbol=WSTETH`, `underlyingTokens=[wstETH addr]`) and then computed `final_asset_in = asset_in or _pick_asset_in(meta)`. When the user supplied `asset_in=USDC`, the explicit value won unconditionally — no check that USDC matched any of the pool's deposit tokens. Downstream the build_yield_execution_plan request carried `asset_in=USDC` while the pool is WSTETH-only, producing a plan whose title labeled it USDC and whose calldata would have routed against WSTETH.
- **Fix**: `src/agent/tools/execute_pool_position.py` — inserted ASSET_POOL_MISMATCH preflight directly after `final_asset_in = asset_in or _pick_asset_in(meta)`. When `asset_in` is explicit (user-supplied, not picker default), extract `_pool_legs` from `meta.symbol` (split on `-/_`), refuse with structured `ASSET_POOL_MISMATCH` blocker if `_user_asset not in _pool_legs`. Stable-to-stable substitution is allowed (USDT → USDC pool both stables: Jupiter/Curve bridge).
- **Pin test**: `tests/defi/test_runtime_invariants.py::test_asset_pool_mismatch_refuses_usdc_against_wsteth_pool` (verbatim WSTETH+USDC case from AI Bug Convo.md), `::test_asset_pool_match_allows_usdc_against_usdc_pool` (sanity that legitimate Aave V3 USDC supply still works), `::test_asset_pool_match_stables_interchangeable` (USDT→USDC pool not refused) — 3 tests, all green.
