...
### BUG-BE-003 — `/api/portfolio/<sol>,<evm>` returns 500 when Moralis SPL quota exhausted
- **Surfaced by**: 2026-05-21 Playwright `docs/playwright-runs/20260521_083415/F03_no_balance` + `20260521_091034/F03_no_balance` + `20260521_091034/B01_stable_strategy` (reproducible, not intermittent as first thought)
- **Severity**: P0 — portfolio endpoint hard-fails when the Solana side hits Moralis rate limit; the wallet-assistant returns 500 with no partial-data fallback, taking down the entire dashboard tab even when only the SPL side failed
- **Spec reference**: §10 wallet/portfolio UI
- **Root cause**: `[SOL] Moralis SPL error: {"message":"Validation service blocked: Your plan: free-plan-daily total included usage has been consumed"}` in `ilyonai-staging-assistant-api-1` logs. The wallet-assistant's SPL path treats Moralis 4xx as fatal, returns 500 to the caller. Direct `curl` returns 200 because the response is cached for a few minutes — under Playwright load (which fires several concurrent requests + drops the cache via fresh contexts) the 500 surfaces.
- **Fix (deferred — needs wallet-assistant code change)**: in `IlyonAi-Wallet-assistant-main/server/app/api/portfolio.py`, treat Moralis SPL failure as a partial — return EVM tokens + a `partial:true` flag + the error message rather than 5xx. Optionally add a 5-min in-memory cache layer in the wallet-assistant itself so repeated requests within the window reuse the last successful payload even if SPL is rate-limited.
- **Pin test (deferred)**: needs a wallet-assistant integration test that mocks Moralis 4xx and asserts portfolio returns 200 + `partial:true`.
