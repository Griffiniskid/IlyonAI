...
- 2026-05-19 `<sha-pending>` (V7-013 closure, Wave 2 SA5): range numeric reconciliation + spec-path shim.
  - **Numeric reconciliation.** Verified `RangePreset.WIDE = 2500` bps (±25%) at `src/agent/intent/liquidity_intent.py:91-100` matches spec PDF §3.3 p.6 verbatim: *"Wide ±25% (~4× efficiency, ~95% in-range historically for blue-chips)"*. BALANCED=1000 bps (±10%), TIGHT=500 bps (±5%). Stable-pair table at `:112-119` matches §3.3 "stable pairs use ±0.5% / ±0.1% / ±0.05%" (50/10/5 bps).
  - **No `spec.md` exists** in the repo (`glob **/spec.md` returns no files). The "spec.md says WIDE=±20%" concern from the V8 resume brief was a phantom — the only spec source of truth is `IlyonAi_LP_Execution_Spec.pdf`, and code already aligns. No code change to canonical module.
  - **Spec-path shim created** at `src/defi/range_calculator.py` — thin re-export of `RangePreset`, `RANGE_PRESET_BPS`, `STABLE_RANGE_PRESET_BPS`, `range_preset_bps` from the canonical module. Satisfies spec-mandated import path without duplicating logic.
  - **Pin tests** at `tests/defi/test_range_calculator_shim.py` (8 tests): shim importability, numeric values (WIDE=2500, BALANCED=1000, TIGHT=500), and identity guards (`shim_fn is canonical_fn`, same enum, same dict objects) to prevent silent divergence.
- 2026-05-19 `<sha-pending>` (V7-020 closure, Wave 2 SA8): OpenRouter file-path drift resolved via spec-path shim.
  - **Canonical implementation unchanged.** The dual-provider LLM client lives at `src/ai/openai_client.py::OpenAIClient` (constructor at `:33-58`). A single class drives both OpenAI and OpenRouter via the `use_openrouter: bool` flag — `use_openrouter=True` selects `base_url="https://openrouter.ai/api/v1/chat/completions"`, `api_key=settings.openrouter_api_key`, `model=settings.ai_model`, and adds `HTTP-Referer` / `X-Title` headers at request time (`:873-875`). There is no separate `OpenRouterClient` class because the flag-based design avoids duplicate code paths.
  - **Spec-path shim created** at `src/agent/llm/openrouter_client.py` — re-exports `OpenAIClient` from the canonical module so spec-style `from src.agent.llm.openrouter_client import …` imports work without duplicating logic. Also exports `OpenRouterClient = OpenAIClient` as a clearer alias for callers that want explicit semantics. The alias IS `OpenAIClient` — same class object, not a subclass.
  - **Package init** at `src/agent/llm/__init__.py` — docstring-only; the `src/agent/` package already exists.
  - **Pin tests** at `tests/defi/test_openrouter_shim.py` (5 tests, all green in 0.40s): spec-import resolves, alias is importable, `OpenRouterClient is OpenAIClient` (identity), shim class IS canonical class (drift guard), and smoke construction with `use_openrouter=True` flips `base_url` to the OpenRouter endpoint and pins `api_key`/`model`/`use_openrouter` flag.
  - **No change to canonical `src/ai/openai_client.py`.** The dual-provider design is correct as-is per V7-020 spec intent (single client, provider flag).
