...
def _get_balance_via_portfolio(addr_input: str, user_address: str = "", solana_address: str = "") -> str:
    ...
    """
    addr = addr_input.strip().split("|")[0].strip() or user_address or solana_address
    if not addr:
...
• get_defi_analytics — USE THIS for ANY question about APY, APR, yield, best pools, or earning.
  Searches DefiLlama for yield-bearing pools. Returns APY, TVL, and protocol links.
  Input: token symbol(s), chain, protocol. Add "sort:apy" when user asks for "highest APR/APY".
  Examples: "USDC polygon", "SOL solana sort:apy", "USDT USDC sort:apy".
  CRITICAL: If user asks "highest APR pool for X/Y", "best yield for X", "where to earn on X"
  → ALWAYS use get_defi_analytics with "sort:apy", NOT find_liquidity_pool.
  IMPORTANT: Do NOT add a chain filter unless the user explicitly names a chain.
  The connected network is irrelevant for yield searches — search all chains by default.
...
- APR/APY pool search: ALWAYS use get_defi_analytics (with sort:apy for "highest"), never find_liquidity_pool
...
- Add liquidity to a pool (provide liquidity, deposit into LP)
- Stake tokens / unstake tokens
...
A. TRANSACTIONS (build_solana_swap / build_swap_tx / build_transfer_tx):
...
C. SIMPLE QUESTIONS & ANALYTICS (no lists of entities):
   Final Answer is plain text in the user's language. You may use **bold** and emoji.
...
- CRITICAL JSON PASSTHROUGH: When a tool returns JSON with a "type" field (like "balance_report", \
  "universal_cards", "evm_action_proposal", "liquidity_pool_report", "solana_swap_proposal"), \
...
def get_defi_analytics(query: str) -> str:
    ...
    try:
        ...
        # Parse query into token(s) / chain / protocol / sort filters
        parts = [part.strip('"\' ,') for part in query.strip().split() if part.strip('"\' ,')]
        token_filters: list[str] = []  # support multiple tokens for pair search (e.g. USDT + USDC)
        chain_filter = ""
        protocol_filter = ""
        ...
        for part in parts:
            ...
            if lower.startswith("sort:"):
                ...
                continue
            # Slash-separated pair: "USDT/USDC" → two token filters
            if "/" in part:
                token_filters.extend(t.upper() for t in part.split("/") if t.strip())
                continue
            mapped_chain = _CHAIN_ALIASES_LLAMA.get(lower, "")
            if mapped_chain:
                chain_filter = mapped_chain
            elif lower in _KNOWN_PROTOCOLS:
        ...
        # TVL floors: always at least $100K for APY sort (prevents tiny high-APY anomalies),
        ...
        for pool in all_pools:
            ...
            if token_filters:
                # Split pool symbol into individual token components for exact matching.
                # e.g. "USDC-USDT Pool" → {"USDC", "USDT"}
                # This prevents bridged variants like USDC.E, IUSDT, WNUSDC from matching
                # a search for plain USDT/USDC.
                sym_clean = sym.replace(" POOL", "").replace(" LP", "")
                pool_tokens = set(re.split(r"[-+/]", sym_clean))
                if not all(t in pool_tokens for t in token_filters):
                    continue
