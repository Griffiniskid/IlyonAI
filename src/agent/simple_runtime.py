...
_ENSO_PROTOS_RE = re.compile(
    ...
    r"marinade(?:[ -]?(?:liquid[ -]?staking|finance))?|msol|"
    r"jito(?:[ -]?(?:liquid[ -]?staking|sol|stake[ -]?pool))?|jitosol|"
    r"sanctum(?:[ -]?(?:infinity|inf|router))?|"
    r"blazestake|blaze|drift[ -]?staking|drift[ -]?staked[ -]?sol|dsol|"
    r"jupiter[ -]?perps|jlp|"
    r"raydium(?:[ -]?(?:amm|clmm|cpmm))?|"
    r"orca(?:[ -]?(?:dex|whirlpool|whirlpools))?|"
    r"meteora(?:[ -]?(?:dlmm|damm[ -]?v?2?|dynamic[ -]?(?:vaults?|amm)))?|"
    r"kamino(?:[ -]?(?:lend|liquidity|vaults?))?|"
    r"morpho(?:[ -]?blue)?|"
    r"yearn(?:[ -]?finance|[ -]?v[23])?|"
    r"compound(?:[ -]?v?\d)?|"
    r"aave(?:[ -]?v?\d)?|"
    r"spark(?:[ -]?savings|[ -]?protocol)?|sdai|usds|"
    r"sky(?:[ -]?lending|[ -]?savings)?|"
    r"morpho|silo(?:[ -]?v\d)?|"
    r"pendle(?:[ -]?v\d)?|pt[ -]?\w+|yt[ -]?\w+|"
    r"uniswap(?:[ -]?v[234])?|"
    r"pancakeswap(?:[ -]?(?:amm[ -]?)?v[23])?|"
    r"sushiswap|"
...
_STRATEGY_SCRATCHPAD_LEAD_RE = re.compile(
    r"^\s*("
    r"we\s+(?:need|have|must|should|will|can|can'?t|cannot|don'?t)\b|"
    r"i\s+(?:need|will|should|must|think|will)\b|"
    r"i'?ll\s+(?:think|compose|build|produce|consider|set|use|do|map|allocate)|"
    r"let'?s\s+(?:think|compose|build|produce|consider|compute|do|map|allocate)|"
    r"let\s+me\s+(?:think|compose|build|produce|consider|compute|do|map|allocate)|"
    r"the\s+user\s+(?:wants|asked|wants|says|said|requested)|"
    r"based\s+on\s+the\s+(?:data|live|prior|previous)|"
    r"to\s+(?:answer|produce|build|compose|allocate|distribute)|"
    r"need\s+to\s+(?:produce|build|compose|answer|allocate|distribute|decide|follow)|"
    # Matrix Pass A wave 3 — additional CoT/scratchpad lead-in shapes
    # observed in B14/A05/C07 final.content leaks.
    r"sum[\s:=]|"               # "Sum: 153.7 + ..."
    r"avg(?:erage)?[\s:=]|"     # "Average = 323.7 / 8 = ..."
    r"compute[\s:]|"            # "Compute: ..." / "compute exact"
    r"default\s+to\s+|"         # "DEFAULT to EVEN split..."
    r"the\s+instruction[:.]|"   # "The instruction: ..."
    r"output\s+format|"         # "OUTPUT FORMAT — markdown:"
    r"however,?\s+(?:we|i)\b|"  # "However we must..."
    r"i\s+think\s+(?:we|you)|"  # "I think we should..."
    r"probably\s+we\b|"
    r"we'?ll\s+(?:set|use|do|map|allocate|compute|assign)|"
    r"\d+(?:\.\d+)?\s*[+\-*×x/÷]\s*\d|"  # bare arithmetic "153.7 + 144.1" / "13.13*7"
    r"first[,\s]|"
    r"alright[,\s]|"
    r"okay[,\s]|"
    r"sure[,\s]|"
